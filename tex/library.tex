...
\lstinputlisting{./code/chinese_postman.cpp}
\subsubsection{全点対間最短路(Johnson)}
$O(max(VE\log V, V^2))$
\lstinputlisting{./code/johnson.cpp}
\subsubsection{無向グラフの全域最小カット}
$O(V^3)$
\lstinputlisting{./code/minimum_cut.cpp}
...
$O(V MAXFLOW)$
\lstinputlisting{./code/gomory_hu.cpp}
...
$O(VE)$
\lstinputlisting{./code/ms_arborescence.cpp}
...
順序付き$O(n)$\par
順序なし$O(n\log n)$
\lstinputlisting{./code/tree_isomorphism.cpp}
...
\lstinputlisting{./code/miller_rabin.cpp}
...
\lstinputlisting{./code/closest_pair.cpp}
\subsection{点-多角形包含判定}
$O(n)$
\lstinputlisting{./code/contains.cpp}
\subsection{凸多角形の共通部分}
$O(n+m)$
\lstinputlisting{./code/convex_intersect.cpp}
\subsection{凸多角形の直径}
$O(n)$
\lstinputlisting{./code/convex_diameter.cpp}
\subsection{ドロネー三角形分割(逐次添加法)}
$O(n^2)$
\lstinputlisting{./code/delaunay.cpp}
